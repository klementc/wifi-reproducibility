...
int main(int argc, char** argv)
{
  ...
  while(std::getline(stream, line)){
    ...
    simgrid::kernel::resource::NetworkWifiLink* l= (simgrid::kernel::resource::NetworkWifiLink *)simgrid::s4u::Link::by_name(apName.str())->get_impl();
...
void updateAllLinksTh() {
  ...
  for (auto const* link : eng->get_all_links()){
    ...
    const simgrid::kernel::resource::NetworkWifiLink* wifi_link = nullptr;
    if(link->get_sharing_policy() == simgrid::s4u::Link::SharingPolicy::WIFI) {
      wifi_link = static_cast<simgrid::kernel::resource::NetworkWifiLink*>(link->get_impl());
    }
    ...
    while (const auto* var = wifi_link->get_constraint()->get_variable(&elem)) {
      auto* action = static_cast<simgrid::kernel::resource::NetworkWifiAction*>(var->get_id());

...
run_once() {
    ...
    [ "$type" = "rand" ] && { python platform_generator.py rand ${nbCells} ${minSTAPerCell} ${maxSTAPerCell} ${AP_BW} ${seedP} ${logs}/${args}; }
    [ "$type" = "norm" ] && { python platform_generator.py norm ${nbCells} ${meanStaPerCell} ${devStaPerCell} ${AP_BW} ${seedP} ${logs}/${args}; }
    ...
    tStart=$(date "+%s")
    /usr/bin/time -f 'swappedCount:%W cpuTimeU:%U cpuTimeS:%S wallClock:%e peakMemUsage:%M' make -sC $sg ARGS="${logs}/${args}_sg_platf.xml ${logs}/flows_${args}.csv ${simTime}" run |& tee $sgout >&2
    tSG=$(( $(date "+%s") - tStart))
